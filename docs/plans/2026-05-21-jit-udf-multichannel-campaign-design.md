...
- **Structref-backed UDAF pattern** merged upstream as [Goykhman/numbduck#24](https://github.com/Goykhman/numbduck/pull/24) (2026-04-22) — bridges DuckDB's aggregate lifecycle to numba structref state via `borrow_structref` + `_deref_structref_raw_ptr` intrinsics. Design doc at [`test/test_ducklib.md`](../../test/test_ducklib.md).
...
4. **UDAF pattern** — structref state + 6 lifecycle callbacks (`size`/`init`/`update`/`combine`/`finalize`/`destroy`) + bridge intrinsics (`borrow_structref`, `_deref_structref_raw_ptr`). Link to [`test/test_ducklib.md`](../../test/test_ducklib.md) for the full design. Note `removerefctpass` interaction. IRR example as the worked use case.
...
| Title candidates | "From Flying to Machine Code: JIT'd DuckDB UDFs at C Speed"; "Answering a 2022 Feature Request: Vectorized Python UDFs Without the Python"; "Numba-JIT'd UDFs in DuckDB: Three Problems Dissolved" |
...
5. **Aggregates: the UDAF pattern** — short walkthrough of the IRR example. State as numba structref via [`make_structref`](https://github.com/Goykhman/numbox/blob/main/numbox/utils/highlevel.py); six lifecycle callbacks; result fetched via `duckdb_fetch_chunk`. Link to [`test/test_ducklib.md`](../../test/test_ducklib.md) and [`examples/irr.py`](../../examples/irr.py) for depth.
...
| Author voice | nelson2005 |
| Title candidates (HN) | "Removing the Python boundary from DuckDB UDFs (1750× faster than Arrow at 1M rows)"; "How we JIT-compiled DuckDB UDFs and got 400×–1750× over Python and Arrow"; "DuckDB UDFs that run at C speed without crossing into Python" |
...
5. **Aggregates: the "wait, even *that*?" moment** — UDAF is harder than scalar because state has to survive across chunks and threads. numbduck uses [numbox structref](https://github.com/Goykhman/numbox) for the state; six lifecycle callbacks bridge DuckDB's aggregate protocol. Walk through IRR (Internal Rate of Return) as the worked example: accumulate `(cashflow, period)` pairs, bisect for the rate. Link to [`examples/irr.py`](https://github.com/Goykhman/numbduck/blob/main/examples/irr.py).
