...
- **Motivating examples**: Goykhman/numbduck#21 — upstream PR open, Goykhman commented with benchmark results from his arm64 Mac (2026-04-11). We replied with CI benchmark numbers (x86 + arm64) for comparison. His Python 3.12 scalar UDF at 10K rows (0.005s) is anomalous vs CI (0.785s) — awaiting his response. Feature branch: `motivating-examples-spec`, upstream PR branch: `motivating-examples-upstream`. Fork PR: nelson2005/numbduck#28. Fork CI: all 48 jobs passed (2026-04-11). Upstream CI: all 12 jobs passed.
- **Copilot review comments on #21**: 3 comments — unused `time_median` import (fixed), unused `import numba` (fixed), unchecked return codes in `@njit(nogil=True)` loop (pushed back: `raise` doesn't work without GIL, controlled inputs in benchmark). All replied to.
- **`_jit_clock.monotonic_ns` rewritten as `@intrinsic`**: Replaced `@cres` + caller-allocated scratch buffer with zero-arg LLVM `@intrinsic` using stack `alloca`. Eliminates NRT liveness hack. Committed on both feature and upstream branches. Proposed as numbox utility: Goykhman/numbox#7.
- **Haversine benchmark CI workflow**: `.github/workflows/haversine_bench.yml` — fork-only `workflow_dispatch` workflow for running full haversine benchmarks (NUMBDUCK_BENCH_BIG=1) on x86 + arm64 with matched library versions (numpy 1.26.4, pyarrow 14.0.0). Added to `main` to enable dispatch. Results posted to Goykhman's PR comment.
