...
- **IRR UDAF example + vector container**: branch `feat/irr-udaf-example`. Fork PR [nelson2005/numbduck#35](https://github.com/nelson2005/numbduck/pull/35) open. Adds `numbduck/vector.py` (generic numba vector container with `make_vector`), capacity-only constructor API (`Float64Vec(64)`), 14 vector tests, and `examples/irr.py` (IRR aggregate using vector state). Spec: [`docs/specs/2026-04-17-irr-udaf-example-design.md`](docs/specs/2026-04-17-irr-udaf-example-design.md). Fork-only — no upstream PR planned (example).
