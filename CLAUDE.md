...
- **Structref-backed UDAF feature**: branch `feat/structref-udaf-stddev`. All 4 plan tasks complete. Fork PR [nelson2005/numbduck#34](https://github.com/nelson2005/numbduck/pull/34) open, CI green (44/44 jobs, run 24592490726). Upstream PR [Goykhman/numbduck#24](https://github.com/Goykhman/numbduck/pull/24) submitted from branch `upstream/structref-udaf-stddev` (single squashed commit, only `test/test_ducklib.py`, no fork-only files). Bot review comments on fork PR #34 addressed: Gemini NRT stats comment pushed back (incorrect), Copilot c_size_t fix applied, Copilot in-scope refcount suggestion investigated and documented as impossible (removerefctpass strips NRT calls), doc/code mismatches fixed. Added `refcount_of_meminfo` intrinsic (commented out) as cautionary example with detailed explanation of why it can't observe in-scope refcounts. numbox comparison: `numbox/utils/meminfo.py` has `structref_meminfo` (extracts MemInfo ptr without incref) and `get_nrt_refcount` (Python-side refcount read) — no equivalents for borrow/release ownership transfer. Plan: `docs/plans/2026-04-15-structref-udaf-stddev-plan.md`. Next: await upstream review on Goykhman/numbduck#24.
