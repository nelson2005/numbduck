...
- **Structref-backed UDAF feature**: branch `feat/structref-udaf-stddev` (latest `4f1e7c4`). All 4 plan tasks complete. Fork PR [nelson2005/numbduck#34](https://github.com/nelson2005/numbduck/pull/34) open. Upstream PR [Goykhman/numbduck#24](https://github.com/Goykhman/numbduck/pull/24) from branch `upstream/structref-udaf-stddev` (latest `f7d2c25`). Two new Copilot bot comments on upstream PR (2026-04-18) re-raising the `removerefctpass` concern about `export_meminfo`/`borrow_structref` — already replied with symmetric-stripping analysis. Awaiting human review from upstream maintainer.
- **IRR UDAF example**: branch `feat/irr-udaf-example` (latest `3b9e40e`). Implementation complete. Fork PR [nelson2005/numbduck#35](https://github.com/nelson2005/numbduck/pull/35) open. Self-contained example in `examples/irr.py` using numbox `make_structref` for state, bisection solver, all 6 aggregate callbacks, multi-group verification, NRT leak check. Spec: `docs/specs/2026-04-17-irr-udaf-example-design.md`. Plan: `docs/plans/2026-04-17-irr-udaf-example.md`. DuckDB Python API has no `create_aggregate_function` equivalent — custom aggregates require C API access. PR review (Gemini + Copilot) addressed 2026-04-18: NULL validity checks added, extend() in combine, memsys_enable_stats moved to main(), validity uint64→intp cast, doc reference fixes, numbox version fix. Pushed back on Gemini's `irr_state_type` removal (wrong — `borrow_structref` needs type instance not proxy class) and Copilot's state_size portability (64-bit only). All 14 comments replied to. Next: check CI results (run `24597044784` queued), no upstream PR planned (fork-only example).
