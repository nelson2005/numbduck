...
- **Structref-backed UDAF feature**: branch `feat/structref-udaf-stddev` (latest `21a81fa`). All 4 plan tasks complete. Fork PR [nelson2005/numbduck#34](https://github.com/nelson2005/numbduck/pull/34) open. Upstream PR [Goykhman/numbduck#24](https://github.com/Goykhman/numbduck/pull/24) from branch `upstream/structref-udaf-stddev` (latest `f7d2c25`); includes `test/test_ducklib.py` + `test/test_ducklib.md` (design doc). Bot review comments addressed on both PRs: fork PR #34 (Gemini NRT stats pushed back, Copilot c_size_t fixed, refcount_of_meminfo cautionary tale added); upstream PR #24 (two Copilot removerefctpass comments — pushed back with detailed symmetric-stripping analysis and links to numba source). Design doc `test/test_ducklib.md` added explaining DuckDB aggregate lifecycle, bridge intrinsics, removerefctpass interaction, and `@cfunc`/`@njit` callback pattern. Plan: `docs/plans/2026-04-15-structref-udaf-stddev-plan.md`. Next: await upstream review on Goykhman/numbduck#24.
