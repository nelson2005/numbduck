...
- **In flight (2026-04-13):**
  ...
  - nelson2005/numbduck#31 — fork sync PR: merges upstream/main (`3c4837f` "small stylistic improvements") into fork main without force-pushing main. Branch `sync/upstream-main`. **CI all CANCELLED (not failed)** — likely concurrency group superseded. Investigate/re-run before merge.
  - nelson2005/numbduck#30 — closed (changes belonged upstream, not on fork main). Branch `feat/use-numbox-clock` is the fork's feature branch with the same changes + fork-only commit (CLAUDE.md, numbduck_ci.yml, haversine_bench.yml) on top. Tip `bc1dc3d` mirrors the upstream review fix, committed with `[skip ci]` (doc-only).
