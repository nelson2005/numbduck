...
- **IRR UDAF example + vector container**: branch `feat/irr-udaf-example` (latest `9f017f3`). Fork PR [nelson2005/numbduck#35](https://github.com/nelson2005/numbduck/pull/35) open — CI run `24618489396` in progress (2026-04-19). Also has branch `feat/vector-type` (same commit, no PR). Changes since last session: added `numbduck/vector.py` (generic numba vector container with `make_vector`), capacity-only constructor API (`Float64Vec(64)` — dtype locked by type), 14 vector tests, irr.py fixes (uint64 index casts, combine logic, bisection ordinal exponents, 120-char reformat, cache clearing docs). Spec: `docs/specs/2026-04-17-irr-udaf-example-design.md`. No upstream PR planned (fork-only example).
  - **Next steps**: (1) Check CI results on PR #35, (2) delete `feat/vector-type` branch once CI passes (merged into irr branch), (3) consider whether vector container should move to numbduck core or stay example-local, (4) address any new PR review comments.
