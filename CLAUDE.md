...
- **IRR UDAF example**: branch `feat/irr-udaf-example` (latest `4c42a18`). Implementation complete. Fork PR [nelson2005/numbduck#35](https://github.com/nelson2005/numbduck/pull/35) open, CI running. Self-contained example in `examples/irr.py` using numbox `make_structref` for state, bisection solver, all 6 aggregate callbacks, multi-group verification, NRT leak check. Spec: `docs/specs/2026-04-17-irr-udaf-example-design.md`. Plan: `docs/plans/2026-04-17-irr-udaf-example.md`. DuckDB Python API has no `create_aggregate_function` equivalent — custom aggregates require C API access. Next: check CI results, no upstream PR planned yet (example is fork-only for now).
